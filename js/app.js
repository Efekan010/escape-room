// Shared game logic for all rooms
const MAX_LIVES = 3;
let totalBoxes = 0;
let solvedBoxes = 0;
...
function loadTeamName() {
  let team = localStorage.getItem('darkHouseTeam');
  if (team && document.querySelector('.team-label')) {
    document.querySelector('.team-label').innerHTML = `👥 Team: ${team}`;
  }
}
...
function checkAnswer() {
  ...
  if (userAnswer === correctAnswer.toLowerCase()) {
    ...
    if (livesLeft <= 0) {
      ...
      feedback.style.color = '#cc2222';
      document.getElementById('answer').style.borderColor = '#cc2222';
      setTimeout(() => document.getElementById('answer').style.borderColor = '', 400);
      document.getElementById('answer').value = '';
...
function renderLives() {
  ...
  el.innerHTML = `<span style="color:var(--red-light)">${full}</span><span style="color:#3a2a2a">${empty}</span>`;
}
...
function triggerJumpscare() {
  ...
  setTimeout(() => {
    if (scare) scare.classList.add('active');
  }, 80);
}
